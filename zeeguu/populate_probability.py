...
def set_default_exercise_based_prob():
    ...
    for bookmark in bookmarks:
        prob = model.ExerciseBasedProbability.find(bookmark.user, bookmark.origin, default_probability)
        zeeguu.db.session.add(prob)
    zeeguu.db.session.commit()

def set_default_encounter_based_prob(lang_code):
    zeeguu.app.test_request_context().push()
    zeeguu.db.session.commit()
    default_probability = 0.5
    lang = model.Language.find(lang_code)
        ...
        for word_known in words_known_from_user:
            if model.WordRank.exists(word_known.lower(), lang):
               ...
               if model.EncounterBasedProbability.exists(user, rank):
                   ...
                   prob.count_not_looked_up = prob.count_not_looked_up + 1
               else:
                   prob = model.EncounterBasedProbability.find(user,rank)
                   zeeguu.db.session.add(prob)
...
if __name__ == "__main__":
    ...
    set_default_encounter_based_prob('de')
